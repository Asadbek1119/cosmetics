<?php

use common\models\BlogCategory;
use common\models\BlogNews;
use yii\helpers\Html;
use yii\helpers\Url;
use yii\grid\ActionColumn;
use yii\grid\GridView;
use yii\widgets\Pjax;
/* @var $this yii\web\View */
/* @var $searchModel common\models\search\BlogNewsSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('app', 'Yangiliklar');
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="blog-news-index">

    <p>
        <?= Html::a("<i class='fas fa-plus'></i> ".Yii::t('app', 'Yaratish'), ['create'], ['class' => 'btn btn-success']) ?>
    </p>

    <?php Pjax::begin(); ?>
    <?php // echo $this->render('_search', ['model' => $searchModel]); ?>

    <?= GridView::widget([
        'dataProvider' => $dataProvider,
        'filterModel' => $searchModel,
        'columns' => [
            ['class' => 'yii\grid\SerialColumn'],

            //'id',
            [
                    'attribute' => 'image',
                    'format' => ['image',['width'=>'60px','height'=>'60px']]
            ],
            //'theme',
            //'views',
            //'date',
            'created_at',
            'updated_at',
            [
                'attribute' => 'status',
                'filter' => BlogNews::getStatusFilter(),
                'value' => static function(BlogNews $model){
                    return $model->getStatusLabel();
                }
            ],
            [
                'label' => 'Kategoriya nomi',
                'attribute' => 'blog_category_id',
                'filter' => BlogCategory::categories(),
                'value' => static function(BlogNews $model){
                    return $model->blogCategory->name;
                }
            ],
            ['class' => 'yii\grid\ActionColumn'],
        ],
    ]); ?>

    <?php Pjax::end(); ?>

</div>
